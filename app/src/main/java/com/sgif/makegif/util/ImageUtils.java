...
public final class ImageUtils {
    ...
    public static List<FolderImage> getAllImage(Context context) {
        if (ActivityCompat.checkSelfPermission(context, Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            return null;
        }

        List<Photo> mPhotos = new ArrayList<>();
        List<FolderImage> mList = new ArrayList<>();
        ...
        cc.close();
        mList.addAll(getAlbumImage(mPhotos));
        return mList;
    }

    private static List<FolderImage> getAlbumImage(List<Photo> photos) {
    ...
    private static boolean checkAlbum(List<FolderImage> folderImages, Photo photo) {
        boolean check = false;
